...
def test_auto_import_chromium_invalid_file_keeps_other_source(bukuDb, chromium_profile, invalid_filename, capsys, caplog):
    profile = chromium_profile()
    for filename in ['Bookmarks', 'AccountBookmarks']:
    ...
    assert 'Could not import bookmarks from Google Chrome' not in capsys.readouterr().out
    errors = [msg for _, level, msg in caplog.record_tuples if level == 40]
    assert len(errors) == 1 and str(profile / invalid_filename) in errors[0]


def test_auto_import_chromium_all_files_invalid(bukuDb, chromium_profile, capsys, caplog):
    profile = chromium_profile()
    for filename in ['Bookmarks', 'AccountBookmarks']:
        (profile / filename).write_text('not JSON', encoding='utf-8')
    bdb = bukuDb()
    ...
    assert capsys.readouterr().out.count('Could not import bookmarks from Google Chrome') == 1
    errors = [msg for _, level, msg in caplog.record_tuples if level == 40]
    assert len(errors) == 2
    assert {str(profile / filename) in msg for filename, msg in zip(['Bookmarks', 'AccountBookmarks'], errors)} == {True}
